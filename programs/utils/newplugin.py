...
target_link_libraries( {plugin_provider}_{plugin_name} zattera_app zattera_chain zattera_protocol )
...
{{
   public:
      {plugin_name}_api( const zattera::app::api_context& ctx );
...
#include <zattera/app/plugin.hpp>
...
class {plugin_name}_plugin : public zattera::app::plugin
{{
   public:
      {plugin_name}_plugin( zattera::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <zattera/app/api_context.hpp>
#include <zattera/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( zattera::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      zattera::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( zattera::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const zattera::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( zattera::app::application& app );
      ...
      zattera::app::application& _app;
      boost::signals2::scoped_connection _applied_block_conn;
}};

{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( zattera::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( zattera::app::application* app )
   : plugin(app)
{{
...
ZATTERA_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
