...
{
   zattera::protocol::transaction     tx;
   std::string                      wif;
};
...
struct tx_signing_result
{
   ...
   zattera::protocol::public_key_type key;
   zattera::protocol::signature_type  sig;
};
...
int main(int argc, char** argv, char** envp)
{
   ...
   while( std::cin )
   {
      ...
      try
      {
         ...
         auto priv_key = zattera::utilities::wif_to_key( sreq.wif );

         if(priv_key)
         {
            sres.sig = priv_key->sign_compact( sres.sig_digest );
            sres.key = zattera::protocol::public_key_type( priv_key->get_public_key() );
