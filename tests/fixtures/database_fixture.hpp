...
#include <zattera/plugins/block_api/block_api_plugin.hpp>
#include <zattera/plugins/database_api/database_api_plugin.hpp>
...
#define PUSH_BLOCK \
   zattera::chain::test::_push_block
...
/*#define ZATTERA_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "ZATTERA_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "ZATTERA_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}*/
...
#define ZATTERA_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "ZATTERA_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "ZATTERA_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   ZATTERA_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   ZATTERA_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
#define ASSET( s ) \
   zattera::chain::test::asset_from_string( s )
...
{ \
   signed_transaction tx; \
   OP2TX(OP,tx,KEY) \
   ZATTERA_REQUIRE_THROW( db->push_transaction( tx, 0 ), EXCEPTION ); \
}
...
struct database_fixture {
   ...
   string debug_key = zattera::utilities::key_to_wif( init_account_priv_key );
...
{
   private:
      zattera::plugins::json_rpc::json_rpc_plugin* rpc_plugin;
