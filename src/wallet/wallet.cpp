...
#include <zattera/utils/words.hpp>

#include <zattera/protocol/base.hpp>
#include <zattera/wallet/wallet.hpp>
#include <zattera/wallet/api_documentation.hpp>
...
public:
   ...
   {
      string client_version( zattera::utilities::git_revision_description );
      const size_t pos = client_version.find( '/' );
      if( pos != string::npos && client_version.size() > pos )
         client_version = client_version.substr( pos + 1 );

      fc::mutable_variant_object result;
      result["blockchain_version"  ] = ZATTERA_BLOCKCHAIN_VERSION;
      result["client_version"      ] = client_version;
      result["zattera_revision"    ] = zattera::utilities::git_revision_sha;
      result["zattera_revision_age"] = fc::get_approximate_relative_time_string( fc::time_point_sec( zattera::utilities::git_revision_unix_timestamp ) );
      ...
      try
      {
         ...
         result["server_blockchain_version"] = config["ZATTERA_BLOCKCHAIN_VERSION"];
   ...
   bool import_key(string wif_key)
   {
      ...
      zattera::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         zattera::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         zattera::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         zattera::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         zattera::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < ZATTERA_MAX_TIME_UNTIL_EXPIRATION );
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["list_my_accounts"] = [](fc::variant result, const fc::variants& a ) {
         ...
         for( const auto& a : accounts ) {
            ...
            out << std::left << std::setw( 17 ) << std::string(a.name)
                ...
                << std::right << std::setw(26) << fc::variant(a.vesting_share_balance).as_string() <<" "
      ...
      m["get_order_book"] = []( fc::variant result, const fc::variants& a ) {
         ...
         ss << ' ' << setw( ( spacing * 4 ) + 6 ) << "Bids" << "Asks\n"
            << ' '
            << setw( spacing + 3 ) << "Sum(ZBD)"
            << setw( spacing + 1) << "ZBD"
            << setw( spacing + 1 ) << "ZTR"
            << setw( spacing + 1 ) << "Price"
            << setw( spacing + 1 ) << "Price"
            << setw( spacing + 1 ) << "ZTR "
            << setw( spacing + 1 ) << "ZBD " << "Sum(ZBD)"
            << "\n====================================================================================================="
            << "|=====================================================================================================\n";
...
brain_key_info wallet_api::suggest_brain_key()const
{
   ...
   {
      fc::bigint choice = entropy % zattera::words::word_list_size;
      entropy /= zattera::words::word_list_size;
      if( i > 0 )
         brain_key += " ";
      brain_key += zattera::words::word_list[ choice.to_int64() ];
   }
...
vector< database_api::api_withdraw_vesting_route_object > wallet_api::get_withdraw_routes( string account, zattera::wallet::withdraw_route_type type )const
{
   ...
   bool broadcast )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * ZATTERA_1_PERCENT;
