...
using namespace zattera::utilities;
...
class wallet_api
{
   public:
       ...
       */
      vector< database_api::api_withdraw_vesting_route_object > get_withdraw_routes( string account, zattera::wallet::withdraw_route_type type = zattera::wallet::all )const;
      ...
      /**
       * Transfer funds from one account to another. ZTR and ZBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 ZTR"
      ...
      /**
       * Transfer funds from one account to another using escrow. ZTR and ZBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
      ...
      /**
       * Transfer ZTR into a vesting fund represented by vesting shares (VESTS). VESTS are required to vesting
       * for a minimum of one coin year and can be withdrawn once a week over a two year withdraw period.
       * VESTS are protected against dilution up until 90% of ZTR is vesting.
       *
       * @param from The account the ZTR is coming from
       * @param to The account getting the VESTS
       * @param amount The amount of ZTR to vest i.e. "100.00 ZTR"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *    withdrawn and deposited back as ZTR. i.e. "10.000000 VESTS"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *
       * @param from The account the VESTS are withdrawn from.
       * @param to   The account receiving either VESTS or ZTR.
      ...
      /**
       *  This method will convert ZBD to ZTR at the current_median_history price one
       *  week from the time it is executed. This method depends upon there being a valid price feed.
       *
       *  @param from The account requesting conversion of its ZBD i.e. "1.000 ZBD"
      ...
      /**
       * A witness can public a price feed for the ZTR:ZBD market. The median price feed is used
       * to process conversion requests from ZBD to ZTR.
       *
       ...
       *                       operations defined in `zattera/chain/operations.hpp`
      ...
      /**
       * Gets the current order book for ZTR:ZBD
       *
       ...
       *  @param amount_to_sell The amount of either ZBD or ZTR you wish to sell
      ...
      /**
       * Vote on a comment to be paid ZTR
       *
...
FC_REFLECT( zattera::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( zattera::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( zattera::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( zattera::wallet::authority_type, (owner)(active)(posting) )

FC_API( zattera::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
...
FC_REFLECT( zattera::wallet::memo_data, (from)(to)(nonce)(check)(encrypted) )
