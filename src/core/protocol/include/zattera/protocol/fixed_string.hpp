...
namespace fc { namespace raw {
   ...
   void to_variant(   const zattera::protocol::fixed_string_impl< Storage >& s, variant& v ) { v = std::string( s ); }

   template< typename Storage >
   void from_variant( const variant& v, zattera::protocol::fixed_string_impl< Storage >& s ) { s = v.as_string(); }
} // fc
