...
zattera::protocol::transaction_id_type zattera::protocol::transaction::id() const
{
   auto h = digest();
...
const signature_type& zattera::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)
{
...
signature_type zattera::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)const
{
   digest_type::encoder enc;
   fc::raw::pack( enc, chain_id );
   fc::raw::pack( enc, *this );
   ...
   uint32_t max_account_auths )const
{
   ...
   for( const public_key_type& k : s )
   {
      ...
      try
      {
         zattera::protocol::verify_authority(
            operations,
            result,
            get_active,
            get_owner,
