...
{
   class variant;
   inline void to_variant( const zattera::chain::shared_string& s, variant& var )
   {
      var = fc::string( zattera::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, zattera::chain::shared_string& s )
   {
   ...
   namespace raw {
      ...
      template< typename T >
      inline T unpack_from_vector( const zattera::chain::buffer_type& s )
      { try  {
         T tmp;
         if( s.size() ) {
...
#ifndef ENABLE_STD_ALLOCATOR
FC_REFLECT_TYPENAME( zattera::chain::shared_string )
#endif

FC_REFLECT_ENUM( zattera::chain::bandwidth_type, (post)(forum)(market) )
