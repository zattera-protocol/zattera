...
void reset_virtual_schedule_time( database& db )
{
   ...
   for( const auto& witness : idx )
   {
      db.modify( witness, [&]( witness_object& wobj )
      {
         ...
         wobj.virtual_scheduled_time = ZATTERA_VIRTUAL_SCHEDULE_LAP_LENGTH2 / (wobj.votes.value+1);
      } );
   }
}
...
void update_witness_schedule4( database& db )
{
   ...
   active_witnesses.reserve( ZATTERA_MAX_WITNESSES );
   ...
   for( auto witness_count = selected_voted.size();
        sitr != schedule_idx.end() && witness_count < ZATTERA_MAX_WITNESSES;
        ++sitr )
   {
   ...
   {
      auto new_virtual_scheduled_time = new_virtual_time + ZATTERA_VIRTUAL_SCHEDULE_LAP_LENGTH2 / ((*itr)->votes.value+1);
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      for( size_t i = active_witnesses.size(); i < ZATTERA_MAX_WITNESSES; i++ )
      {
...
{
   if( (db.head_block_num() % ZATTERA_MAX_WITNESSES) == 0 ) //wso.next_shuffle_block_num )
   {
