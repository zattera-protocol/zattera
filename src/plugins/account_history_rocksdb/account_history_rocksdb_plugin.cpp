#include <zattera/plugins/account_history_rocksdb/account_history_rocksdb_plugin.hpp>
...
#define ZATTERA_NAMESPACE_PREFIX "zattera::protocol::"
...
namespace zattera { namespace plugins { namespace account_history_rocksdb {

using zattera::protocol::account_name_type;
using zattera::protocol::block_id_type;
using zattera::protocol::operation;
using zattera::protocol::signed_block;
using zattera::protocol::signed_block_header;
using zattera::protocol::signed_transaction;

using zattera::chain::operation_notification;
using zattera::chain::transaction_id_type;

using zattera::utilities::benchmark_dumper;
...
public:
   impl( account_history_rocksdb_plugin& self, const bpo::variables_map& options, const bfs::path& storagePath) :
      _self(self),
      _mainDb(appbase::app().get_plugin<zattera::plugins::chain::chain_plugin>().db()),
      _storagePath(storagePath),
      _writeBuffer(_storage, _columnHandles)
      {
      collectOptions(options);

      _mainDb.add_pre_reindex_handler([&]( const zattera::chain::reindex_notification& note ) -> void
         {
            on_pre_reindex( note );
         }, _self, 0);

      _mainDb.add_post_reindex_handler([&]( const zattera::chain::reindex_notification& note ) -> void
         {
   ...
   void on_pre_reindex( const zattera::chain::reindex_notification& note );
   void on_post_reindex( const zattera::chain::reindex_notification& note );
...
void account_history_rocksdb_plugin::impl::collectOptions(const boost::program_options::variables_map& options)
{
   ...
   ZATTERA_LOAD_VALUE_SET(options, "account-history-rocksdb-track-account-range", _tracked_accounts, pairstring);
...
std::vector<account_name_type> account_history_rocksdb_plugin::impl::getImpactedAccounts(const operation& op) const
{
   ...
   zattera::app::operation_get_impacted_accounts(op, impacted);
...
void account_history_rocksdb_plugin::impl::storeOpFilteringParameters(const std::vector<std::string>& opList,
   flat_set<std::string>* storage) const
   {
      for(const auto& arg : opList)
      {
         ...
         {
            if( op.empty() == false )
               storage->insert( ZATTERA_NAMESPACE_PREFIX + op );
         }
      }
   }
...
void account_history_rocksdb_plugin::impl::on_pre_reindex(const zattera::chain::reindex_notification& note)
{
...
void account_history_rocksdb_plugin::impl::on_post_reindex(const zattera::chain::reindex_notification& note)
{
...
FC_REFLECT( zattera::plugins::account_history_rocksdb::account_history_info,
