#include <zattera/plugins/market_history_api/market_history_api_plugin.hpp>
...
{
   public:
      market_history_api_impl() : _db( appbase::app().get_plugin< zattera::plugins::chain::chain_plugin >().db() ) {}
...
DEFINE_API_IMPL( market_history_api_impl, get_market_history_buckets )
{
   ...
   result.bucket_sizes = appbase::app().get_plugin< zattera::plugins::market_history::market_history_plugin >().get_tracked_buckets();
   return result;
}
...
{
   JSON_RPC_REGISTER_API( ZATTERA_MARKET_HISTORY_API_PLUGIN_NAME );
}
