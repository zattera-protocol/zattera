...
#include <zattera/plugins/network_broadcast_api/network_broadcast_api_plugin.hpp>
   ...
   class network_broadcast_api_impl
   {
      public:
         network_broadcast_api_impl() :
            _p2p( appbase::app().get_plugin< zattera::plugins::p2p::p2p_plugin >() ),
            _chain( appbase::app().get_plugin< zattera::plugins::chain::chain_plugin >() )
         {}
         ...
         zattera::plugins::p2p::p2p_plugin&                      _p2p;
         zattera::plugins::chain::chain_plugin&                  _chain;
   };
...
{
   JSON_RPC_REGISTER_API( ZATTERA_NETWORK_BROADCAST_API_PLUGIN_NAME );
}
