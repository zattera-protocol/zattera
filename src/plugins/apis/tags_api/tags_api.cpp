#include <zattera/plugins/tags_api/tags_api_plugin.hpp>
#include <zattera/plugins/tags_api/tags_api.hpp>
#include <zattera/plugins/tags/tags_plugin.hpp>
#include <zattera/plugins/follow_api/follow_api_plugin.hpp>
...
{
   public:
      tags_api_impl() : _db( appbase::app().get_plugin< zattera::plugins::chain::chain_plugin >().db() ) {}
      ...
      chain::database& _db;
      std::shared_ptr< zattera::plugins::follow::follow_api > _follow_api;
};
...
DEFINE_API_IMPL( tags_api_impl, get_discussions_by_promoted )
{
   ...
   auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type( ZATTERA_MAX_SHARE_SUPPLY ) )  );
...
void tags_api_impl::set_pending_payout( discussion& d )
{
   ...
   if( itr != cidx.end() && itr->comment == d.id )  {
      d.promoted = asset( itr->promoted_balance, DOLLAR_SYMBOL );
   }
   ...
   if( d.parent_author != ZATTERA_ROOT_POST_PARENT )
                                                        ...
                                                        )
{
   ...
   while( count > 0 && tidx_itr != tidx.end() )
   {
      ...
      try
      {
         ...
         result.discussions.back().promoted = asset(tidx_itr->promoted_balance, DOLLAR_SYMBOL );
...
{
   JSON_RPC_REGISTER_API( ZATTERA_TAGS_API_PLUGIN_NAME );
}
...
{
   auto follow_api_plugin = appbase::app().find_plugin< zattera::plugins::follow::follow_api_plugin >();
