...
#include <zattera/plugins/block_data_export/exportable_block_data.hpp>

#include <zattera/plugins/stats_export/stats_export_plugin.hpp>
...
namespace zattera { namespace plugins { namespace stats_export {

using zattera::chain::block_notification;
using zattera::chain::database;
using zattera::chain::dynamic_global_property_object;
...
using zattera::plugins::block_data_export::exportable_block_data;

using zattera::plugins::chain::chain_plugin;
...
FC_REFLECT( zattera::plugins::stats_export::detail::api_stats_transaction_data_object, (user)(size) )
FC_REFLECT( zattera::plugins::stats_export::detail::api_stats_export_data_object, (global_properties)(transaction_stats)(free_memory) )

namespace zattera { namespace plugins { namespace stats_export { namespace detail {
...
{
   std::shared_ptr< api_stats_export_data_object > stats = _export_plugin.find_export_data< api_stats_export_data_object >( ZATTERA_STATS_EXPORT_PLUGIN_NAME );
   if( !stats )
      return;
...
void stats_export_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      my->_export_plugin.register_export_data_factory( ZATTERA_STATS_EXPORT_PLUGIN_NAME,
