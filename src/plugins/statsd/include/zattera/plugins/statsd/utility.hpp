#pragma once
#include <zattera/plugins/statsd/statsd_plugin.hpp>

#include <fc/optional.hpp>
#include <fc/time.hpp>

namespace zattera { namespace plugins { namespace statsd { namespace util {

using zattera::plugins::statsd::statsd_plugin;
...
} } } } // zattera::plugins::statsd::util

#define STATSD_INCREMENT( NAMESPACE, STAT, KEY, FREQ )   \
if( zattera::plugins::statsd::util::statsd_enabled() )     \
{                                                        \
   zattera::plugins::statsd::util::get_statsd().increment( \
...
if( zattera::plugins::statsd::util::statsd_enabled() )     \
{                                                        \
   zattera::plugins::statsd::util::get_statsd().decrement( \
      #NAMESPACE, #STAT, #KEY, FREQ                      \
   );                                                    \
}
...
if( zattera::plugins::statsd::util::statsd_enabled() )     \
{                                                        \
   zattera::plugins::statsd::util::get_statsd().count(     \
...
if( zattera::plugins::statsd::util::statsd_enabled() )     \
{                                                        \
   zattera::plugins::statsd::util::get_statsd().gauge(     \
      #NAMESPACE, #STAT, #KEY, VAL, FREQ                 \
   );                                                    \
}

#define STATSD_START_TIMER( NAMESPACE, STAT, KEY, FREQ )                                              \
fc::optional< zattera::plugins::statsd::util::statsd_timer_helper > NAMESPACE ## STAT ## KEY ## _timer; \
if( zattera::plugins::statsd::util::statsd_enabled() )                                                  \
{                                                                                                     \
   NAMESPACE ## STAT ## KEY ## _timer = zattera::plugins::statsd::util::statsd_timer_helper(            \
      #NAMESPACE, #STAT, #KEY, FREQ, zattera::plugins::statsd::util::get_statsd()                       \
   );                                                                                                 \
}
...
if( zattera::plugins::statsd::util::statsd_enabled() )     \
{                                                        \
   zattera::plugins::statsd::util::get_statsd().timing(    \
      NAMESPACE, STAT, KEY,                              \
      zattera::plugins::statsd::util::timing_helper( VAL ),\
      FREQ                                               \
   );                                                    \
}
