...
#include <zattera/plugins/block_data_export/block_data_export_plugin.hpp>

#include <zattera/plugins/witness/witness_export_objects.hpp>
...
#include <zattera/chain/utils/impacted.hpp>

#include <zattera/utils/key_conversion.hpp>
#include <zattera/utils/plugin_utilities.hpp>
...
namespace zattera { namespace plugins { namespace witness {

using chain::plugin_exception;
using std::string;
using std::vector;
using zattera::plugins::block_data_export::block_data_export_plugin;
...
namespace detail {
   ...
   public:
      witness_plugin_impl( appbase::io_service_t& io ) :
         _timer(io),
         _chain_plugin( appbase::app().get_plugin< zattera::plugins::chain::chain_plugin >() ),
         _db( appbase::app().get_plugin< zattera::plugins::chain::chain_plugin >().db() )
         {}
      ...
      uint32_t _required_witness_participation = 33 * ZATTERA_1_PERCENT;
      uint32_t _production_skip_flags = chain::database::skip_nothing;

      std::map< zattera::protocol::public_key_type, fc::ecc::private_key > _private_keys;
      std::set< zattera::protocol::account_name_type >                     _witnesses;
      ...
      std::set< zattera::protocol::account_name_type >                     _dupe_customs;
   ...
   struct comment_options_extension_visitor
   {
      ...
      {
         ZATTERA_ASSERT( cpb.beneficiaries.size() <= 8,
            plugin_exception,
   ...
   void check_memo( const string& memo, const chain::account_object& account, const account_authority_object& auth )
   {
      ...
      {
         for( auto& key : keys )
            ZATTERA_ASSERT( key_weight_pair.first != key,  plugin_exception,
      ...
      for( auto& key : keys )
         ZATTERA_ASSERT( memo_key != key,  plugin_exception,
   ...
   struct operation_visitor
   {
      ...
      void operator()( const comment_operation& o )const
      {
         if( o.parent_author != ZATTERA_ROOT_POST_PARENT )
         {
            ...
            if( parent != nullptr )
            ZATTERA_ASSERT( parent->depth < ZATTERA_SOFT_MAX_COMMENT_DEPTH,
               plugin_exception,
               "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",ZATTERA_SOFT_MAX_COMMENT_DEPTH) );
         }
      }
      ...
      switch( note.op.which() )
      {
         ...
         case operation::tag< custom_binary_operation >::value:
         {
            ...
            for( auto& account : impacted )
               if( _db.is_producing() )
                  ZATTERA_ASSERT( _dupe_customs.insert( account ).second, plugin_exception,
   ...
   { try {
      ...
      if( BOOST_UNLIKELY( reserve_ratio_ptr == nullptr ) )
      {
         _db.create< reserve_ratio_object >( [&]( reserve_ratio_object& r )
         {
            ...
            r.current_reserve_ratio = ZATTERA_MAX_RESERVE_RATIO * RESERVE_RATIO_PRECISION;
            r.max_virtual_bandwidth = ( static_cast<uint128_t>( ZATTERA_MAX_BLOCK_SIZE) * ZATTERA_MAX_RESERVE_RATIO
                                       * ZATTERA_BANDWIDTH_PRECISION * ZATTERA_BANDWIDTH_AVERAGE_WINDOW_SECONDS )
                                       / ZATTERA_BLOCK_INTERVAL;
         });
            ...
            */
            if( _db.head_block_num() % 20 == 0 )
            {
               ...
               else
               {
                  ...
                  if( r.current_reserve_ratio > ZATTERA_MAX_RESERVE_RATIO * RESERVE_RATIO_PRECISION )
                     r.current_reserve_ratio = ZATTERA_MAX_RESERVE_RATIO * RESERVE_RATIO_PRECISION;
               }
               ...
               r.max_virtual_bandwidth = ( uint128_t( max_block_size ) * uint128_t( r.current_reserve_ratio )
                                          * uint128_t( ZATTERA_BANDWIDTH_PRECISION * ZATTERA_BANDWIDTH_AVERAGE_WINDOW_SECONDS ) )
                                          / ( ZATTERA_BLOCK_INTERVAL * RESERVE_RATIO_PRECISION );
            }
         });
      }

      std::shared_ptr< exp_witness_data_object > export_data =
         zattera::plugins::block_data_export::find_export_data< exp_witness_data_object >( ZATTERA_WITNESS_PLUGIN_NAME );
      if( export_data )
   ...
   void witness_plugin_impl::update_account_bandwidth( const chain::account_object& a, uint32_t trx_size, const bandwidth_type type )
   {
      ...
      if( props.total_vesting_shares.amount > 0 )
      {
         ...
         share_type trx_bandwidth = trx_size * ZATTERA_BANDWIDTH_PRECISION;
         auto delta_time = ( _db.head_block_time() - band->last_bandwidth_update ).to_seconds();

         if( delta_time > ZATTERA_BANDWIDTH_AVERAGE_WINDOW_SECONDS )
            new_bandwidth = 0;
         else
            new_bandwidth = ( ( ( ZATTERA_BANDWIDTH_AVERAGE_WINDOW_SECONDS - delta_time ) * fc::uint128( band->average_bandwidth.value ) )
               / ZATTERA_BANDWIDTH_AVERAGE_WINDOW_SECONDS ).to_uint64();
         ...
         if( _db.is_producing() )
            ZATTERA_ASSERT( has_bandwidth,  plugin_exception,
               "Account: ${account} bandwidth limit exceeded. Please wait to transact or power up ZTR.",
               ("account", a.name)
   ...
   {
      if( fc::time_point::now() < fc::time_point(ZATTERA_GENESIS_TIME) )
      {
         wlog( "waiting until genesis time to produce block: ${t}", ("t",ZATTERA_GENESIS_TIME) );
   ...
   block_production_condition::block_production_condition_enum witness_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture)
   {
      ...
      {
         capture("pct", uint32_t(100*uint64_t(prate) / ZATTERA_1_PERCENT));
      ...
      appbase::app().get_plugin< zattera::plugins::p2p::p2p_plugin >().broadcast_block( block );
...
{ try {
   ...
   if( export_plugin != nullptr )
   {
      ...
      export_plugin->register_export_data_factory( ZATTERA_WITNESS_PLUGIN_NAME,
         []() -> std::shared_ptr< exportable_block_data > { return std::make_shared< exp_witness_data_object >(); } );
   }

   ZATTERA_LOAD_VALUE_SET( options, "witness", my->_witnesses, zattera::protocol::account_name_type )
   ...
   if( options.count("private-key") )
   {
      ...
      {
         fc::optional<fc::ecc::private_key> private_key = zattera::utilities::wif_to_key(wif_key);
   ...
   {
      my->_required_witness_participation = ZATTERA_1_PERCENT * options.at( "required-participation" ).as< uint32_t >();
   }
...
{ try {
   ...
   chain::database& d = appbase::app().get_plugin< zattera::plugins::chain::chain_plugin >().db();

   if( !my->_witnesses.empty() )
   {
      ilog( "Launching block production for ${n} witnesses.", ("n", my->_witnesses.size()) );
      appbase::app().get_plugin< zattera::plugins::p2p::p2p_plugin >().set_block_production( true );
