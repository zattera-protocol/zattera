...
struct post_operation_visitor
{
   ...
   void operator()( const custom_json_operation& op )const
   {
      try
      {
         if( op.id == ZATTERA_FOLLOW_PLUGIN_NAME )
         {
...
void follow_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   try
   {
      ...
      _custom_operation_interpreter = std::make_shared< generic_custom_operation_interpreter< zattera::plugins::follow::follow_plugin_operation > >( my->_db );
