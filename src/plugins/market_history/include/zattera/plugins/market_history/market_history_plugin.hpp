...
// time.
//
#ifndef ZATTERA_MARKET_HISTORY_SPACE_ID
#define ZATTERA_MARKET_HISTORY_SPACE_ID 7
#endif

#ifndef ZATTERA_MARKET_HISTORY_PLUGIN_NAME
#define ZATTERA_MARKET_HISTORY_PLUGIN_NAME "market_history"
#endif


namespace zattera { namespace plugins { namespace market_history {
...
{
   bucket_object_type        = ( ZATTERA_MARKET_HISTORY_SPACE_ID << 8 ),
   order_history_object_type = ( ZATTERA_MARKET_HISTORY_SPACE_ID << 8 ) + 1
};
...
class market_history_plugin : public plugin< market_history_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = ZATTERA_MARKET_HISTORY_PLUGIN_NAME; return name; }
...
FC_REFLECT( zattera::plugins::market_history::bucket_object_details,
            (high)
            (low)
            (open)
            (close)
...
FC_REFLECT( zattera::plugins::market_history::order_history_object,
                     (id)
                     (time)
                     (op) )
CHAINBASE_SET_INDEX_TYPE( zattera::plugins::market_history::order_history_object, zattera::plugins::market_history::order_history_index )
