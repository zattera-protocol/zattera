...
#define ZATTERA_CHAIN_PLUGIN_NAME "chain"
...
public:
   ...
   static const std::string& name() { static std::string name = ZATTERA_CHAIN_PLUGIN_NAME; return name; }
   ...
   bool accept_block( const zattera::chain::signed_block& block, bool currently_syncing, uint32_t skip );
   void accept_transaction( const zattera::chain::signed_transaction& trx );
   zattera::chain::signed_block generate_block(
   ...
   bool block_is_on_preferred_chain( const zattera::chain::block_id_type& block_id );

   void check_time_in_block( const zattera::chain::signed_block& block );
