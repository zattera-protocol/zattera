...
#include <zattera/plugins/statsd/utility.hpp>

#include <zattera/utils/benchmark_dumper.hpp>
...
using fc::flat_map;
using zattera::chain::block_id_type;
...
const zattera::chain::database& chain_plugin::db() const { return my->db; }
...
void chain_plugin::plugin_startup()
{
   ...
   {
      auto statsd = appbase::app().find_plugin< zattera::plugins::statsd::statsd_plugin >();
      if( statsd != nullptr )
      {
   ...
   zattera::utilities::benchmark_dumper dumper;

   const auto& abstract_index_cntr = my->db.get_abstract_index_cntr();

   typedef zattera::utilities::benchmark_dumper::index_memory_details_cntr_t index_memory_details_cntr_t;
   ...
   db_open_args.liquid_initial_supply = ZATTERA_LIQUID_INITIAL_SUPPLY;
   db_open_args.dollar_initial_supply = ZATTERA_DOLLAR_INITIAL_SUPPLY;
      ...
      const chainbase::database::abstract_index_cntr_t& abstract_index_cntr )
   {
      ...
      {
         typedef zattera::utilities::benchmark_dumper::database_object_sizeof_cntr_t database_object_sizeof_cntr_t;
      ...
      const zattera::utilities::benchmark_dumper::measurement& measure =
   ...
   if(my->replay)
   {
      ...
      db_open_args.benchmark = zattera::chain::database::TBenchmark(my->benchmark_interval, benchmark_lambda);
      last_block_number = my->db.reindex( db_open_args );

      if( my->benchmark_interval > 0 )
      {
         const zattera::utilities::benchmark_dumper::measurement& total_data = dumper.dump(true, get_indexes_memory_details);
   ...
   {
      db_open_args.benchmark = zattera::chain::database::TBenchmark(dump_memory_details, benchmark_lambda);
      ...
      catch( const fc::exception& e )
      {
         ...
         catch( zattera::chain::block_log_exception& )
         {
...
bool chain_plugin::accept_block( const zattera::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
void chain_plugin::accept_transaction( const zattera::chain::signed_transaction& trx )
{
   boost::promise< void > prom;
   write_context cxt;
   cxt.req_ptr = &trx;
...
zattera::chain::signed_block chain_plugin::generate_block(
   const fc::time_point_sec when,
   const account_name_type& witness_owner,
   const fc::ecc::private_key& block_signing_private_key,
   uint32_t skip )
...
bool chain_plugin::block_is_on_preferred_chain(const zattera::chain::block_id_type& block_id )
{
   ...
   return db().get_block_id_for_num( zattera::chain::block_header::num_from_id( block_id ) ) == block_id;
}

void chain_plugin::check_time_in_block( const zattera::chain::signed_block& block )
{
...
} } } // namespace zattera::plugis::chain::chain_apis
