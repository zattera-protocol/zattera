...
All plugins in the `src/plugins` directory are iterated over by `CMakeLists.txt`. The manifest directory iterates through all plugins, adding them to the `zattera_plugins` build target. Any other build target wanting to access all plugins available at build time should link to this target.
...
- By default, zatterad runs the `chain`, `p2p`, and `webserver` plugins
...
- [Testing Guide](testing.md) - Running tests and code coverage
